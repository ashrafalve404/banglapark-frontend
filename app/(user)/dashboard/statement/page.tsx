...
import { statementApi } from "@/lib/api/statement";
...
export default function StatementPage() {
    ...
    const downloadPdf = async () => {
        ...
        try {
            ...
            const txt = (
                ...
                x: number,
                yPos: number,
                opts?: { font?: string; size?: number; color?: number[]; align?: string }
            ) => {
            ...
            const labelValue = (label: string, value: string) => {
                ...
                txt(value, margin + 70, y, { size: 9, color: [31, 41, 55] });
                y += 5.5;
            };
            ...
            sectionHeading(t("dashboard.statement.financialOverview"));
            ...
            labelValue(t("dashboard.statement.totalWithdrawn"), formatCurrency(ac.totalWithdrawn, locale));
            labelValue(t("dashboard.statement.totalDeposited"), formatCurrency(ac.totalDeposited, locale));
            y += 3;

            txt(t("dashboard.statement.earningsCategory"), margin, y, { font: boldFont, size: 9, color: [22, 163, 74] });
            y += 5.5;
            ...
            labelValue(t("dashboard.statement.digitalMarketingIncome"), formatCurrency(ac.digitalMarketingIncome, locale));
            labelValue(t("dashboard.statement.sellerIncome"), formatCurrency(ac.sellerIncome, locale));
            labelValue(t("dashboard.statement.positionSalary"), formatCurrency(ac.positionSalary, locale));
            labelValue(t("dashboard.statement.quizIncome"), formatCurrency(ac.quizIncome, locale));
            labelValue(t("dashboard.statement.transferIn"), formatCurrency(ac.transferIn, locale));
            y += 3;

            txt(t("dashboard.statement.expensesCategory"), margin, y, { font: boldFont, size: 9, color: [220, 38, 38] });
            y += 5.5;
            labelValue(t("dashboard.statement.digitalMarketingSpent"), formatCurrency(ac.digitalMarketingSpent, locale));
            labelValue(t("dashboard.statement.totalSpent"), formatCurrency(data.orders.totalSpent, locale));
            labelValue(t("dashboard.statement.giftCardSpent"), formatCurrency(ac.giftCardSpent, locale));
            labelValue(t("dashboard.statement.cpaSpent"), formatCurrency(ac.cpaSpent, locale));
            labelValue(t("dashboard.statement.transferOut"), formatCurrency(ac.transferOut, locale));
            y += 4;

            // ── Team & Activity Summary ──
            sectionHeading(t("dashboard.statement.teamSection"));
            labelValue(t("dashboard.statement.totalTeam"), `${data.team.totalTeam}`);
            labelValue(t("dashboard.statement.directTeam"), `${data.team.directTeam}`);
            labelValue(t("dashboard.statement.activeTeam"), `${data.team.activeTeam}`);
            y += 4;
    ...
    const mainBalances = [
        { label: t("dashboard.statement.totalBalance"), value: ac.walletBalance, icon: Wallet, color: "text-green-700 bg-green-50 border-green-200" },
        { label: t("dashboard.statement.withdrawable"), value: ac.withdrawable, icon: CreditCard, color: "text-emerald-700 bg-emerald-50 border-emerald-200" },
        { label: t("dashboard.statement.pendingWithdrawal"), value: ac.pendingWithdrawal, icon: ArrowDownToLine, color: "text-amber-700 bg-amber-50 border-amber-200" },
        { label: t("dashboard.statement.totalWithdrawn"), value: ac.totalWithdrawn, icon: ArrowUpRight, color: "text-slate-700 bg-slate-50 border-slate-200" },
        { label: t("dashboard.statement.totalDeposited"), value: ac.totalDeposited, icon: ArrowDownLeft, color: "text-indigo-700 bg-indigo-50 border-indigo-200" },
    ];
    ...
    const earnings = [
        ...
        { label: t("dashboard.statement.digitalMarketingIncome"), value: ac.digitalMarketingIncome, icon: Zap, color: "text-cyan-700 bg-cyan-50" },
        { label: t("dashboard.statement.sellerIncome"), value: ac.sellerIncome, icon: ShoppingBag, color: "text-emerald-700 bg-emerald-50" },
        { label: t("dashboard.statement.positionSalary"), value: ac.positionSalary, icon: Briefcase, color: "text-rose-700 bg-rose-50" },
        { label: t("dashboard.statement.quizIncome"), value: ac.quizIncome, icon: HelpCircle, color: "text-yellow-700 bg-yellow-50" },
        { label: t("dashboard.statement.transferIn"), value: ac.transferIn, icon: ArrowDownLeft, color: "text-teal-700 bg-teal-50" },
    ];

    const expenses = [
        { label: t("dashboard.statement.digitalMarketingSpent"), value: ac.digitalMarketingSpent, icon: Zap, color: "text-sky-700 bg-sky-50" },
        { label: t("dashboard.statement.totalSpent"), value: data.orders.totalSpent, icon: ShoppingBag, color: "text-indigo-700 bg-indigo-50" },
        { label: t("dashboard.statement.giftCardSpent"), value: ac.giftCardSpent, icon: Gift, color: "text-pink-700 bg-pink-50" },
        { label: t("dashboard.statement.cpaSpent"), value: ac.cpaSpent, icon: Briefcase, color: "text-orange-700 bg-orange-50" },
        { label: t("dashboard.statement.transferOut"), value: ac.transferOut, icon: ArrowUpRight, color: "text-rose-700 bg-rose-50" },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{t("dashboard.statement.heading")}</h1>
                    <p className="text-xs sm:text-sm text-gray-500">{t("dashboard.statement.subheading")}</p>
                </div>
                <button
                    onClick={downloadPdf}
                    disabled={downloading}
                    className="btn-primary bg-green-700 hover:bg-green-800 flex items-center justify-center gap-2 px-4 py-2.5 text-xs sm:text-sm font-bold self-start w-full sm:w-auto"
                >
                    {downloading ? (
            ...
            <div className="card p-4 sm:p-5">
                <h3 className="text-xs sm:text-sm font-bold text-gray-900 mb-4 pb-2 border-b border-gray-100 flex items-center gap-2">
            ...
            <div className="space-y-4">
                <h3 className="text-xs sm:text-sm font-bold text-gray-900 flex items-center gap-2">
                    ...
                    {t("dashboard.statement.financialOverview")}
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    {mainBalances.map((item) => (
                        <div key={item.label} className="card p-3 text-center border">
                            ...
                            <p className="text-xs sm:text-sm font-extrabold text-gray-800 mt-1">{formatCurrency(item.value, locale)}</p>
                        </div>
                    ))}
                </div>
            </div>

            {/* Financial Breakdown: Earnings vs Outflow */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {/* Earnings & Income */}
                <div className="card p-4 sm:p-5 space-y-3">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                        <h4 className="text-xs sm:text-sm font-bold text-green-700 flex items-center gap-2">
                            <TrendingUp size={16} />
                            {t("dashboard.statement.earningsCategory")}
                        </h4>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-2 gap-2">
                        {earnings.map((item) => (
                            <div key={item.label} className="p-2.5 rounded-lg bg-slate-50 flex items-center gap-2.5 border border-slate-100">
                                <div className={`rounded-md p-1.5 ${item.color} shrink-0`}>
                                    <item.icon size={14} />
                                </div>
                                <div className="min-w-0">
                                    <p className="text-[10px] text-gray-500 font-medium truncate">{item.label}</p>
                                    <p className="text-xs sm:text-sm font-extrabold text-gray-800">{formatCurrency(item.value, locale)}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="card p-4 sm:p-5 space-y-3">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                        <h4 className="text-xs sm:text-sm font-bold text-rose-700 flex items-center gap-2">
                            ...
                            {t("dashboard.statement.expensesCategory")}
                        </h4>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-2 gap-2">
                        {expenses.map((item) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="card p-4 flex items-center gap-3">
                    <div className="rounded-lg bg-green-50 p-2.5 text-green-700 shrink-0">
                        <Users size={20} />
                    </div>
                    <div>
                        <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wide">{t("dashboard.statement.totalTeam")}</p>
                        <p className="text-lg font-extrabold text-gray-800">{data.team.totalTeam}</p>
                    </div>
                </div>
                ...
                <div className="card p-4 flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wide">{t("dashboard.statement.directTeam")}</p>
                        <p className="text-lg font-extrabold text-gray-800">{data.team.directTeam}</p>
                    </div>
                </div>
                ...
                <div className="card p-4 flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wide">{t("dashboard.statement.activeTeam")}</p>
                        <p className="text-lg font-extrabold text-gray-800">{data.team.activeTeam}</p>
                    </div>
                </div>

                <div className="card p-4 flex items-center gap-3">
                    <div className="rounded-lg bg-purple-50 p-2.5 text-purple-700 shrink-0">
                        <ShoppingBag size={20} />
                    </div>
                    <div>
                        <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wide">{t("dashboard.statement.totalOrders")}</p>
                        <p className="text-lg font-extrabold text-gray-800">{data.orders.totalOrders} ({formatCurrency(data.orders.totalSpent, locale)})</p>
                    </div>
                </div>
            </div>
            ...
            <div className="card p-4 sm:p-5">
                ...
                {data.transactions.length === 0 ? (
                    <p className="text-xs sm:text-sm text-gray-400 py-4 text-center">{t("dashboard.statement.noTransactions")}</p>
                ) : (
                        ...
                        <table className="w-full text-xs">
                            ...
                            <tbody>
                                {data.transactions.map((tx, i) => {
                                    const isDebit =
                                        ["PURCHASE", "WITHDRAWAL", "GIFT_CARD_PURCHASE", "CPA_TASK_PURCHASE", "QUIZ_PURCHASE", "QUIZ_DEDUCTION", "TRANSFER_OUT", "DIGITAL_MARKETING_PURCHASE"].includes(tx.type) ||
                                    ...
                                    return (
                                        <tr key={tx.id} className={i % 2 === 0 ? "bg-slate-50/50" : ""}>
                                            <td className="px-3 py-2 text-slate-600 font-medium whitespace-nowrap">{new Date(tx.createdAt).toLocaleDateString()}</td>
                                            <td className="px-3 py-2 font-semibold text-slate-800 whitespace-nowrap">
                                                {(() => {
                                                    const key = `dashboard.statement.transactionType.${tx.type}`;
                                                    const translated = t(key as any);
                                                    if (translated && translated !== key) return translated;
                                                    return tx.type.replace(/_/g, " ").replace(/\w\S*/g, (txt) => txt.charAt(0).toUpperCase() + txt.substring(1).toLowerCase());
                                                })()}
                                            </td>
                                            <td className="px-3 py-2 text-slate-600 max-w-[180px] sm:max-w-[250px] truncate">
                                                {(tx.benefitCategory === "BASE" ? `${t("dashboard.statement.dailyReward")} - ` : tx.benefitCategory === "TIER" ? `${t("dashboard.statement.tierBonus")} - ` : "")}{tx.description}
                                            </td>
                                            <td className={`px-3 py-2 text-right font-extrabold whitespace-nowrap ${isDebit ? "text-red-600" : "text-emerald-700"}`}>
                                                {isDebit ? "- " : "+ "}{formatCurrency(Math.abs(Number(tx.amount)), locale)}
                                            </td>
                                            <td className="px-3 py-2 text-right font-bold text-slate-800 whitespace-nowrap">{formatCurrency(tx.balanceAfter, locale)}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
            ...
            <div className="card p-4 sm:p-5">
                ...
                {data.withdrawals.length === 0 ? (
                    <p className="text-xs sm:text-sm text-gray-400 py-4 text-center">{t("dashboard.statement.noWithdrawals")}</p>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                            <thead>
                            ...
                            <tbody>
                                {data.withdrawals.map((w, i) => (
                                    <tr key={w.id} className={i % 2 === 0 ? "bg-green-50/30" : ""}>
                                        <td className="px-3 py-2 text-gray-600 whitespace-nowrap">{new Date(w.createdAt).toLocaleDateString()}</td>
                                        <td className="px-3 py-2 text-right font-bold text-gray-800 whitespace-nowrap">{formatCurrency(w.amount, locale)}</td>
                                        <td className="px-3 py-2 font-semibold text-gray-800 whitespace-nowrap">{w.method}</td>
                                        <td className="px-3 py-2 text-gray-500 whitespace-nowrap">{(w.accountDetails as any)?.number || "-"}</td>
