            ...
            <div className="card p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
                    ...
                    <div><span className="text-gray-500 font-semibold">{t("dashboard.statement.status")}:</span>
                        <span className={`font-bold ${ac.status === "ACTIVE" ? "text-green-700" : "text-red-600"}`}> {ac.status === "ACTIVE" ? t("dashboard.statement.active") : t("dashboard.statement.inactive")}</span>
                    </div>
                            ...
                            <tbody>
                                {data.transactions.map((tx, i) => (
                                    <tr key={tx.id} className={i % 2 === 0 ? "bg-green-50/30" : ""}>
                                        ...
                                        <td className={`px-3 py-2 text-right font-bold ${Number(tx.amount) >= 0 ? "text-green-700" : "text-red-600"}`}>
                                            {formatCurrency(tx.amount, locale)}
                                        </td>
